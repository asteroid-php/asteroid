<?php
	...
	class Autoloader {
		...
		public function load($class) {
			...
			else $namespace = trim(str_replace("\\", "/", $this->namespace), "/") . "/";
			if($namespace == "/") $namespace = "";
			$class = preg_replace("/[^a-z0-9\/]/", "", str_replace("\\", "/", $class));
			...
			if(file_exists($path) && is_file($path) && is_readable($path))
				require_once $path;
			elseif(file_exists($path = strtolower($path)) && is_file($path) && is_readable($path))
				require_once $path;
		}
